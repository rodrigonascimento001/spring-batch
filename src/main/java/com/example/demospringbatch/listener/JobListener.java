...
public class JobListener extends JobExecutionListenerSupport {
    ...
    public void afterJob(JobExecution jobExecution) {
        if(jobExecution.getStatus() == BatchStatus.COMPLETED){
            LOG.info("Finalizou o Job! Verifique os resultados");
            jdbcTemplate.query("SELECT nome,sobrenome,telefone from persona",(rs,row) -> new Persona(rs.getString(1),rs.getString(2),rs.getString(3))).forEach(persona -> LOG.info("Registro <"+persona+">")); ;
        }
